...
Both of the above formulas are valid in the range:

$$0 <= P_n <= 2*P_{n-1}$$



To extend this analysis to an axor with many naxors and paxors, then we start again with the assumption that:

$$\sum\frac{dL_i'}{dP'} = -\sum\frac{dS_i'}{dP'}$$

and 

$$\frac{d}{dP'}\sum(\frac{P'}{P_L_i} * L_i) = -\frac{d}{dP'}\sum(1-\frac{P'}{P_S_i}*S_i)$$

and

$$\sum\frac{L_i}{P_L_i} = \sum\frac{S_i}{P_S_i}$$
(Again excluding the naxors that have gone to zero in value).
...
This contract is based on one described in the Ethereum White Paper. My disclaimer is that I’m not labeling this contract as naïve because I think Vitalik Buterin was naïve in proposing it – rather, I’m labeling it naïve for the purpose of enabling users in the main use case for the app – which is to allow the user to buy into a paxor and have their paxor axo balance increase or decrease according to y.

###Problems with the Naïve Contract (NC)
...
I propose a long term contract (LTC) as one similar in some ways to the NC, but without a deadline at all. Instead, the contract will make appropriate adjustments over time to keep the long and short positions balanced in terms of liabilities, which are the amounts due to the contract parties at any given time. 
...
### Formula for Total Long Balances

The formula for the balance owed a long position for a current price $0 <= P_n <= 2*P_{n-1}$ in terms of $L_0,P_0$ is, assuming that for all $n \in (0, n)$, $0 <= P_n <= 2*P_{n-1}$:

$$L_n = \frac{P_n}{P_0}*L_0$$

The proof for this formula is in Appendix B. 
...
\# number 3 has the advantage of trusting the blocks with the best overall accuracy on the block with determining the transactions. 
...
## Appendix B: Proof for Formula of $L_n$ in terms of $L_0,P_0,P_n$
...
Which fulfills our design requirement articulated at the beginning.